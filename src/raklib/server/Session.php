...
class Session{
	...
	private $openConnectionVerified = false;
	private $openConnectionTwoGot = false;
	...
	private function handleEncapsulatedPacketRoute(EncapsulatedPacket $packet){
		...
		if($id < 0x80){ //internal data packet
			if($this->state === self::STATE_CONNECTING_2){
				...
				//$this->sessionManager->blockAddress($this->address, 300);
				return;
	...
	public function handlePacket(Packet $packet){
		...
		if($this->state === self::STATE_CONNECTED or $this->state === self::STATE_CONNECTING_2){
				...
				if($packet instanceof ACK){
					...
					foreach($packet->packets as $seq){
						if(isset($this->recoveryQueue[$seq])){
							...
							//var_dump('lost ack with seq: ' . $seq);
						}
					}
					...
					foreach($packet->packets as $seq){
						if(isset($this->recoveryQueue[$seq])){
							...
							//var_dump('lost nack with seq: ' . $seq);
						}
					}
				}
			}
			...
			if($packet instanceof OPEN_CONNECTION_REQUEST_1){
				...
				$this->sendPacket($pk);
				if($this->state == self::STATE_CONNECTING_1 && $this->openConnectionTwoGot){
					$this->openConnectionVerified = true;
					return;
				}
				...
				$this->openConnectionTwoGot = true;

				if(!$this->openConnectionVerified) return;
