...
class SessionManager{
	...
	private function tickProcessor(){
		...
		do{
			...
			for($i = 0; $i < 200 && $stream && !$this->shutdown; ++$i){ //if we received a shutdown event, we don't care about any more messages from the event source
				$stream = $this->receiveStream();
			}

			$socket = true;
			for($i = 0; $i < 200 && $socket; ++$i){
